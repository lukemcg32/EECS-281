...
AVL::Node* AVL::insert_node(AVL::Node* node, int datum) {
    ...
    if (datum < node->datum) {
        ...
        if (node->balance() > 1) {
            // if left subtree too deep & child is right-heavy, do left rotation
            if (node->left->balance() < 0) node->left = rotate_left(node->left);
            // do right rotation
            node = rotate_right(node);
        } else if (node->balance() < -1) {
            // if right subtree too deep & child is left-heavy, do right rotation
            if (node->right->balance() > 0) node->right = rotate_right(node->right);
            // do left rotation
            node = rotate_left(node);
        }
    }
    else {
        // datum is greater than or equal to node->datum, insert right
