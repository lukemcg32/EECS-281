...
void MusicLibrary::get_options(int argc, char** argv) {
    ...
    */
        ...
        switch (option) {
            ...
            */
            ...
            default:
                ...
                throw std::runtime_error("Unknown option provided! Use --help or -h for usage information.");
        }
    }
    ...
    // If num_print is less than 1, throw an error.
    if (num_print < 1)
        throw std::runtime_error("Invalid number of songs to print! Must be at least 1.");
}
...
void MusicLibrary::read_data() {
    ...
    while (std::getline(std::cin, song.name, ',')) {
        ...
        std::string plays_str;
        std::getline(std::cin, plays_str);
        song.plays = (unsigned int)std::stoul(plays_str);
    ...
    // Adjust num_print if it exceeds the number of songs read
    if (num_print > int(music.size()))
        num_print = int(music.size());
}
