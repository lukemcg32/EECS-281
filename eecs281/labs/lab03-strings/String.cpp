...
String& String::erase(size_t pos, size_t len) {
    ...
    if (len == npos || len > sz - pos) {
        len = sz - pos; // Adjust len to erase until the end of the string.
    }

    // Move characters down to overwrite the erased portion.
    for (size_t i = pos + len; i <= sz; ++i) {
        cstr[i - len] = cstr[i];
    }

    // Update size of the string after erasing.
    sz -= len;
...
String& String::insert(size_t pos, const String& str) {
    ...
    size_t newSz = sz + str.sz;
    check_allocation(int(newSz + 1)); // check allocation, resizing if necessary.

        // shift chars to the right str.sz times
        for (size_t i = sz; i > pos; --i) {
            cstr[i + str.sz - 1] = cstr[i - 1];
        }
        // Insert the new string's characters.
        for (size_t i = 0; i < str.sz; ++i) {
            cstr[pos + i] = str.cstr[i];
        }

    // Update the size of the string & add null termination
    sz = newSz;
    cstr[sz] = '\0';
...
size_t String::find_first_of(const String& str, size_t pos) {
    if (pos >= sz) {
        return npos; // Return npos if pos is beyond the string length.
    }

    // Iterate through the string starting from pos.
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    for (size_t i = pos + 1; i > 0; --i) {
        size_t index = i - 1;  // Correctly handle zero case and decrement.
        for (size_t j = 0; j < str.sz; ++j) {
            if (cstr[index] == str.cstr[j]) {
                return index;
            }
        }
    }
    
    // If no match is found, return npos.
    return npos;
}
